...
watch_env = gym.make("highway-fast-v0", render_mode='human', config=config1)
...
class PPO:
  ...
  def __init__(self, env, eval_env=None, watch_env= None):
    ...
    self.batch_size = 128
    self.Epochs = 4
    self.steps_per_update = 4096 #512
    ...
    self.observations_count = 0.000001
  ...
  def compute_gae(self, rewards, values, list_ended, last_value):
  ...
  def collect_trajs(self):
    ...
    for _ in range(self.steps_per_update):
      ...
      if terminated or truncated:
        observations, info = self.env.reset()
    return list_observations, actions, rewards, list_ended, log_probabilities, critic_values, observations, (terminated or truncated)
  ...
  def train(self):
    ...
    for i in range(self.training_length):
      ...
      list_observations, actions, rewards, list_ended, log_probabilities, critic_values, last_observation, episode_ended = self.collect_trajs()
      ...
      advantages, returns = self.compute_gae(rewards, critic_values, list_ended, last_value)
    ...
    # Plot these different metric at the end of the training cycle.
