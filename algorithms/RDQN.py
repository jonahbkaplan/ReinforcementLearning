...
class NStepPriorityReplayBuffer:
    ...
    def push(self, S, A, R, S_next, done):
        ...
        # Normal case: Queue is full, pop one and store
        if len(self.n_queue) >= self.n_step:
            self._process_queue_item()

        # Terminal case: Flush the queue
        if done:
...
class RDQN(Agent):
    def __init__(self, env, batch_size=64, gamma=0.83, C=1000, buffer_capacity=10000, n_step=3, zeta=0.95):
        ...
        if isinstance(env.unwrapped.observation_space,gymnasium.spaces.dict.Dict):
            self.state_dim = sum(math.prod(obs.shape) for obs in env.unwrapped.observation_space.values())
        else:
    ...
    def learn(self, S, A, R, S_next):
        ...
        rewards     = torch.tensor(rewards, dtype=torch.float32, device=self.device).unsqueeze(1) # [Batch, 1]
        ...
        with torch.no_grad():
            ...
            Tz = rewards + (1 - dones) * (self.gamma ** self.n_step) * self.q_1.support
            
            Tz = Tz.clamp(min=self.v_min, max=self.v_max)
            
            # Compute the projection indices (b)
            ...
            offset = torch.linspace(0, (self.batch_size - 1) * self.n_atoms, self.batch_size, device=self.device).long().unsqueeze(1).expand(self.batch_size, self.n_atoms)
